...
CREATE TABLE  `mimic-four-377221.R_TimeQ.K001`   AS
    ...
    from (SELECT * FROM `mimic-four-377221.R_TimeP.K007` where stay_id=0 and transfer_id=0) a
    left join (SELECT * FROM `mimic-four-377221.R_TimeP.K007` where stay_id<>0 and transfer_id<>0) b
    on
...
(
SELECT * FROM `mimic-four-377221.R_TimeP.K007` where stay_id<>0 and transfer_id<>0
)
