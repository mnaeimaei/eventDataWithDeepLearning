...
for i,j in zip(list1,list2):
    ...
    CREATE TABLE  `mimic-four-377221.R_TimeP.{New}`   AS
    ...
    from (SELECT * FROM `mimic-four-377221.R_TimeP.{Old}` where stay_id=0 and transfer_id=0) a
    left join (SELECT * FROM `mimic-four-377221.R_TimeP.{Old}` where stay_id<>0 and transfer_id<>0) b
    on
    ...
    (
    SELECT * FROM `mimic-four-377221.R_TimeP.{Old}` where stay_id<>0 and transfer_id<>0
    )
